...
public class PrinterI implements Demo.Printer {
    ...
    public void printString(String s, CallbackPrx callback, com.zeroc.Ice.Current current) {
        ...
        if (s.contains("list clients")) {
            ...
            if (userList.isEmpty()) {
                ...
                System.out.println("\nFin de la lista de clientes registrados\n-------------------\n");
    ...
    private void handleUserMessage(String s, CallbackPrx callback) {
        ...
        // Inicializar una variable para el nombre del remitente
        String sender = "";

        // Eliminar todo el prefijo hasta la palabra "to" (o sea que elimina el nombre
        // del host que va a enviar)
        if (s.contains("to")) {
            // Encontrar el índice de la palabra "to"
            int toIndex = s.toLowerCase().indexOf("to");
            if (toIndex != -1) {

                // Guardar el nombre del remitente antes de eliminarlo
                sender = s.substring(0, toIndex).trim();
                s = s.substring(toIndex + 2).trim(); // Eliminar todo lo que está antes y el "to" para que quede solo el
                                                     // detinatario y el mensaje
            }
        }
        ...
        if (parts.length == 2) {
            ...
            if (userProxies.containsKey(recipient)) {
                ...
                recipientProxy
                        .reportResponse(new Response(0, "Mensaje de " + sender + ": " + message, requestCount.get()));
            } else {
                System.out.println("El usuario " + recipient + " no está registrado.");
                callback.reportResponse(
                        new Response(1, "Error: El usuario " + recipient + " no está registrado.", requestCount.get()));
            }
        } else {
    ...
    private void broadcastMessage(String s, CallbackPrx callback) {
        // Imprimir mensaje para debug
        System.out.println("\nLlegó al método broadcastMessage");
        System.out.println("Mensaje de entrada al método: " + s);

        // Inicializar una variable para el nombre del remitente
        String sender = "";

        // Eliminar todo el prefijo hasta la palabra "BC"
        if (s.contains("BC ")) {
            // Encontrar el índice de la palabra "BC"
            int bcIndex = s.toLowerCase().indexOf("bc "); // Asegúrate de que sea "bc" para que no falle por mayúsculas
            if (bcIndex != -1) {
                // Guardar el nombre del remitente antes de eliminarlo
                sender = s.substring(0, bcIndex).trim();
                // Eliminar "BC " del mensaje
                s = s.substring(bcIndex + 3).trim(); // Asegúrate de eliminar "BC " correctamente
            }
        }

        // Imprimir el estado del sender y el mensaje modificado
        System.out.println("Mensaje ya modificado: " + s);
        System.out.println("Sender: " + sender);

        // Enviar el mensaje a todos los usuarios registrados
        for (String recipient : userList) {
            CallbackPrx recipientProxy = userProxies.get(recipient);

            // Imprimir el destinatario actual para verificar si la lista no está vacía
            // System.out.println("Enviando mensaje a: " + recipient);

            // System.out.println("proxy: "+recipientProxy);

            if (recipientProxy != null && !recipient.equals(sender)) { // No enviar al remitente
                try {
                    // Aquí es donde se construye el mensaje que se envía a los usuarios
                    recipientProxy.reportResponse(new Response(0,
                            "Broadcast from " + sender + ": el mensaje enviado es: " + s, requestCount.get()));
                    System.out.println("Mensaje enviado a " + recipient);
                } catch (Exception e) {
                    System.out.println("Error enviando mensaje a " + recipient + ": " + e.getMessage());
                }
            }
        }

        // Responder al emisor de la solicitud
        callback.reportResponse(new Response(0, "Broadcast enviado: " + s, requestCount.get()));
    }
